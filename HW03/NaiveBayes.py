...
class NaiveBayes:
  ...
  def __init__(self):
    ...
    self.freqTable = collections.defaultdict(lambda: 0)
  ...
  def classify(self, words):
    ...
    """
    classType = ''
    prev_prob_p = float(self.classes["pos"] )/ float(self.classNum)
    prev_prob_n = float(self.classes["neg"] )/ float(self.classNum)
    prob_p = 0
    prob_n = 0
    for word in words:
      prob_p += math.log((self.freqTable[(word, "pos")] + 1.0) / (self.total["pos"] + len(self.vocab)))
      prob_n += math.log((self.freqTable[(word, "neg")] + 1.0) / (self.total["neg"] + len(self.vocab)))
    prob_p += math.log(prev_prob_p)
    prob_n += math.log(prev_prob_n)
