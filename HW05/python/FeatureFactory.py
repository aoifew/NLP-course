...
class FeatureFactory:
	...
	""" TODO: Add your features here """
        #Titlecase
        if re.match("[A-Z][a-z]+", currentWord) is not None:
            features.append("case=Title")

        #Lowercase
        if re.match("[a-z]{2,}", currentWord) is not None:
        ...
        #Apostrophe
        if len (currentWord) > 1 and currentWord[1] == "'":
            features.append("punctuation=apostrophe")

        #Hyphenated
        if re.match("[A-z][a-z]+-[A-Z][a-z]+", currentWord) is not None:
            features.append("punctuation=hyphen")
        ...
        #Posessive
        if position +1 < len(words):
            if words[position+1] == "'s":
                features.append("nextWord=possessive")

        #A.
        #if re.match("[A-Z]\.", currentWord) is not None:
        #    features.append("style=X.")
        
        #Previous word
        #pattern = "[A-Z][a-z]+"
        #if re.match(pattern, currentWord) is not None and re.match(pattern, words[position+1]) is not None:
        #    features.append("PERSON")
